...
const RequestTable = () => {
  ...
  const handlePrintInvoice = (request: RequestData) => {
    const invoiceWindow = window.open("", "Print Invoice", "width=800,height=600");
    if (invoiceWindow) {
      const content = `
        <html>
          <head>
          ...
          <body>
            ...
            <div class="section"><strong>Product:</strong> ${Array.isArray(request["Product-Name"]) ? request["Product-Name"].join(", ") : request["Product-Name"]}</div>
            <div class="section"><strong>Quantity:</strong> ${request["Quantity"]}</div>
            <div class="section"><strong>Time:</strong> ${new Date(request["Time"].seconds * 1000).toLocaleString()}</div>
